...
cdef class MAT_OGDEN(Material):
	...
	cpdef Piola1Stiffness(self,np.ndarray[double,ndim=2] Fe, int ndim):
		...
		if ndim==2:
			Fe3D =  spl.block_diag(Fe,np.eye(1));
		else:
			Fe3D = Fe;
		nd = 3;
		ng = len(self.mu_i);
		...
		# Compute inverse and eigenvalues
		Je = np.linalg.det(Fe3D);
		delta = np.eye(nd)
		Feinv = np.linalg.inv(Fe3D)
		Ce    = Fe3D.T.dot(Fe3D)
		eigVal, eigVec = np.linalg.eigh(Ce)
		...
		domega  = np.zeros(nd+1);		#domega_1/dlambda_1 .. 
		...
		for eid in range(nd):
			for oid in range(len(self.mu_i)):
				...
				#print Je,Fe
				#domega[nd]  -= self.mu_i[oid]/nd* (Je**(-self.beta*self.alpha_i[oid]-1.))
				#ddomega[nd] += self.mu_i[oid]/nd*(self.beta*self.alpha_i[oid]+1.) * Je**(-self.beta*self.alpha_i[oid]-2.)
				domega[nd] += self.K/ng/nd*(             -Je**(-self.beta-1.)+Je**(-1.));
				ddomega[nd]+= self.K/ng/nd*((self.beta+1)*Je**(-self.beta-2.)-Je**(-2.));


		aa,bb,cc = self.GetCoefficients3D(eigVal,IC,omega,domega,ddomega,nd)

		# Compute derivatives of invariants
		dIC = [];
		dIC.append(2*Fe3D);						# = dI1/dFe

		dIC.append(2*IC[0]*Fe3D-2*Fe3D.dot(Ce))		# = dI2/dFe
		...
		ddIC.append(2*IC[0]*np.einsum('ik,jl->ijkl',delta,delta) \
		             +4*np.einsum('ij,kl->ijkl',Fe3D,Fe3D) \
		             -2*np.einsum('ik,jl->ijkl',delta,Ce) \
		             -2*np.einsum('il,jk->ijkl',Fe3D,Fe3D) \
		             -2*np.einsum('ik,jl->ijkl',Fe3D.dot(Fe3D.T),delta))
		...
		if ndim==2:
			kid = [0,1]
			Pe = Pe[np.ix_(kid,kid)]
			Ae = Ae[np.ix_(kid,kid,kid,kid)]

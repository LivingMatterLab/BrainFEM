...
cdef class CONTACT:
	def __init__(self, slaveNodes, masterNodes,penaltyParameter):
		...
		self.slaveIsActive = [False for i in range(self.numSlaveNodes)]
	...
	cpdef FindActiveSlaves(self, DataContainer dc):
		# Collect current and old locations of all master nodes
		mLoc0 = [];
		mLoc = [];
		for m in self.masterNodes:
		...
		for sID in range(self.numSlaveNodes):
			...
			if penDepth>0:
				self.slaveIsActive[sID]=True
				print 'Penetration: ', penDepth,sLoc
			else:
				self.slaveIsActive[sID]=False
	...
	cpdef getHermiteG(self, double g,double alpha,\
		...
		return np.abs(np.inner((x1-xg),tg))
	...
	cpdef checkSlaveContact(self, np.ndarray[double,ndim=1] sLoc, list mLoc, list lenElem, list tangVec, list normVec):
		...
		if penXi>-epsXi and penXi<=alpha and not penID==0:
			...
			#gg = sco.fsolve(self.getHermiteG, penXi/alpha, args=(alpha,sLoc,mLoc[penID-1],mLoc[penID],mLoc[penID+1]) )[0]
			gg = sco.minimize(self.getHermiteG, penXi/alpha, args=(alpha,sLoc,mLoc[penID-1],mLoc[penID],mLoc[penID+1]), method='SLSQP', bounds=((-1,1),)).x
			...
			#gg = sco.fsolve(self.getHermiteG, (penXi-1.)/alpha, args=(alpha,sLoc,mLoc[penID],mLoc[penID+1],mLoc[penID+2]) )[0]
			gg = sco.minimize(self.getHermiteG, (penXi-1.)/alpha, args=(alpha,sLoc,mLoc[penID],mLoc[penID+1],mLoc[penID+2]), method='SLSQP', bounds=((-1,1),)).x
			dep,nvec,tvec = self.getHermiteDepth(gg,alpha,sLoc,mLoc[penID],mLoc[penID+1],mLoc[penID+2])



			if gg<-1 or gg>1:
				print 'WARNING: gg = ', gg, ', penDepth = ', -dep
			else:
	...
	cpdef getPenalty(self,double penDepth):
		...
		#return (np.sign(penDepth)*self.penaltyParameter*penDepth**2,np.sign(penDepth)*2*self.penaltyParameter*penDepth)

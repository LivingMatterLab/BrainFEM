# -*- coding: utf-8 -*-
# CHEXA7 is CHEXA element with one additional dof for density
...
cdef class CHEXA7(Element):
    def __init__(self,nodes,elementDirection,elementProperty):
        super(CHEXA7,self).__init__('CHEXA7',nodes,elementDirection,elementProperty);

        # Compute element rotation matrix T
        self.T = self.RotationMatrix()

    def __str__(self):
        return super(CHEXA7,self).__str__()
    ...
    cpdef RotationMatrix(self):
        cdef np.ndarray[double, ndim=1] ex, ey, ez
    ...
    cpdef BuildElementMatrix(self,DataContainer dc):
        ...
        udof = np.array([ 0,  1,  2,  4,  5,  6, \
                          8,  9, 10, 12, 13, 14, \
                         16, 17, 18, 20, 21, 22, \
                         24, 25, 26, 28, 29, 30]);
        rdof = np.array([3,7,11,15,19,23,27,31]);
        ...
        for ip in range(8):
            ...
            deform = eh.LocalDeformation_H3(r_ip,s_ip,t_ip,r0E,veE[udof])
            N = deform['N']
            dNx = deform['dNx'].T
            J = deform['J'];
            ...
            # Compute deformation gradient
            deform0 = eh.LocalDeformation_H3(r_ip,s_ip,t_ip,r0E,ve0E[udof])
    ...
    cpdef BuildInternalForceVector(self,DataContainer dc):
        ...
        cdef double r_ip,s_ip, t_ip, detJ, Rho,dRhodt
        cdef double detF, detF0, detF_dt
        cdef np.ndarray[np.int64_t,ndim=1] udof, rdof
    ...
    cpdef getVolume(self,DataContainer dc):
        ...
        for ip in range(8):
            ...
            jac = eh.Jacobian_H3(r_ip,s_ip,t_ip,r)
